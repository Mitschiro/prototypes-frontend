        ...
        <v-container style="padding-top: 5%;">
            <v-layout align-center justify-center column >
                ...
                <v-flex v-if="create">
                    ...
                    <h2>Global Category Settings</h2>
                    <v-switch label="Has Items" v-model="template.hasItems" :success="template.hasItems"></v-switch>
                    <v-autocomplete
                        v-if="template.hasItems"
                ...
                <v-flex v-if="update">
                    ...
                    <v-flex v-if="hasItem">
                        <h2>Global Category Settings</h2>
                        <v-switch label="Has Items" v-model="item.hasItems" :success="item.hasItems"></v-switch>
                        <v-autocomplete
                            v-model="item.items"
                            :items="searchCategories"
                            box
    ...
    export default {
        ...
        methods: {
            createItem: async function () {
                try {
                    await axios.post('http://localhost:8888/api/write/new/category', this.template);
                    this.$router.go('/');
                } catch (error) {
                    console.error(error);
                }
            ...
            updateItem: async function () {
                try {
                    await axios.put(`http://localhost:8888/api/update/category/${this.updateItemName}`, this.item);
                    this.$router.go('/');
                } catch (error) {
                    console.error(error);
            ...
            deleteItem: async function () {
                try {
                    await axios.delete(`http://localhost:8888/api/delete/category/${this.deleteItemName}`);
                    this.$router.go('/');
                } catch (error) {
                    console.error(error);
        ...
        watch: {
            'template.hasItems': async function (newV, oldV) {
                if (newV) {
            ...
            'item.hasItems': async function (newV, oldV) {
                if (newV) {
                    let response = await axios.get('http://localhost:8888/api/all/items');
                    this.searchCategories = [];
                    for (const item of response.data) {
            ...
            'updateItemName': async function (newV, oldV) {
                if (newV) {
                    let response = await axios.get(`http://localhost:8888/api/category/${this.updateItemName}`);
